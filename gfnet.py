...
#Mlp : [N, S] ---> [N, P]  S是in_feature； P是put_feature
#Mlp : [N, S] ---> GELU ---> [N, S] ---->fc2 : [N, S] ---> [N, S]
...
class Mlp(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.drop(x)
        return x
#整个MLP做的是2D的Tensor的变换



#Global Filter ：B（batch size） x N (a, b) x C (channel) 
# a, b 应该是 patch size
#将 Bx N (a,b)x C --->Reshape成 B x a x b xC
#(B,a,b,C) ---> .float32 ---> FFT (针对于 a, b，即patch层面的dimension) Dimension Change : [B,a,b,C] ---> [B, a, [b/2]+1, C ]
# torch.view_as_complex(复数)
'''
#>>> x=torch.randn(4, 2)
#>>> x
...
'''
#complex_weight [h , w, dim, 2] --->(view as complex) [h, w ,dim]复数 ---> [14 x 8 x dim]

#self.blocks = (dim = 768, mlp_ratio = 4, h = 14, w = 8, depth = 12)
#complex_weight = [14, 8, 768, 2] --->[14, 8, 768]
class GlobalFilter(nn.Module):
    def __init__(self, dim, h=14, w=8):
        ...
        self.w = w
        self.h = h
#[2, 196, 768] ---> [2, 14, 14, 768] ---> [2, 14, [14/2]+1, 768] = [2, 14, 8, 768]忽略 spatial size的话，相当于忽略batch size [14, 8, 768]
...
# Block这个部分做的是: layer_norm ---> global filter --->(DropOut)----->Layer Norm

# Layer_norm ---> Gloabl Filter ---> Layer_norm --->MLP
...
class Block(nn.Module):
    ...
    def forward(self, x):
        ...
        return x
#patch embeded output: [2, 196, 768]
#经过一层的layer norm : [2, 196, 768]
#经过一层的Global Filter: [2, 196, 768]
#再经过一层的layer norm : [2, 196, 768]
#MLP ：[2, 196, 768*4] ---> [2, 196, 3072] --->[2, 196, 768]
#残差： [2, 196, 768] ---> [2, 196, 768]
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        B, C, H, W = x.shape #assume: x : 2,3,224,224
        # FIXME look at relaxing size constraints
        assert H == self.img_size[0] and W == self.img_size[1], \
            f"Input image size ({H}*{W}) doesn't match model ({self.img_size[0]}*{self.img_size[1]})."
        x = self.proj(x).flatten(2).transpose(1, 2) #([2, 196, 768])
        #self.proj output: 2,768,14,14
        # flatten(2) : 2,768,196
        return x # 2,196,768
...
class GFNet(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=16, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.pos_embed = nn.Parameter(torch.zeros(1, num_patches, embed_dim)) #pos_emd = (1,196,768)
        self.pos_drop = nn.Dropout(p=drop_rate)

        h = img_size // patch_size #h = 14, w = 8
        w = h // 2 + 1
