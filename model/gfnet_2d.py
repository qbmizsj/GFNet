...
class Global_Filter_2d(nn.Module):
    # 9*10*5 9*2 5*5
    def __init__(self, h=18, w=23, dim=1000):
        super().__init__()
        # 生成可学习权重的做法
        # 通过rand初始生成一个指定尺寸的随机tensor，再通过nn.Parameter实现梯度下降
        # d变为w的一半，向下取整+1。这是快速傅立叶变换的要求
        ...
        # 换成2d 
        self.complex_weight = nn.Parameter(torch.randn(
            h, w, dim, 2, dtype=torch.float32) * 1.5)
        self.h = h
        self.w = w
    ...
    def forward(self, x):
        ...
        x = x * weight
        # x = torch.fft.irfftn(x, s=(18, 21, 18), dim=(1, 2, 3), norm='ortho')
        x = torch.fft.irfftn(x, s=(18, 45), dim=(1, 2), norm='ortho')
        x = x.reshape(B, N, C)
        return x
...
class Block(nn.Module):
    ...
    def __init__(self, dim=1000, mlp_ratio=2., drop=0.5, drop_path=0.6, act_layer=nn.GELU, norm_layer=nn.LayerNorm, h=18, w=21):
        super().__init__()
        self.norm1 = norm_layer(dim)
        # self.filter = Global_Filter(dim=1000, h=h, w=w, d=d)
        self.filter = Global_Filter_2d(dim=1000, h=h, w=w)
...
class GFNet(nn.Module):
    ...
    def __init__(self, img_size=(181//2+1, 217//2+1, 181//2+1), patch_size=(10,10,10), embed_dim=1000, num_classes=16, in_channels=1, drop_rate=0.5, depth=1, mlp_ratio=2., representation_size=None, uniform_drop=False, drop_path_rate=0.6, norm_layer=False, dropcls=0.25):
        super().__init__()
