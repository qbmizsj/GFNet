...
class tiny_GF(nn.Module):
    def __init__(self, patch_size, img_size, dim, norm_layer=nn.LayerNorm):
        super().__init__()
        h, w, d = img_size
...
class GFNet(nn.Module):
    ...
    def __init__(self, img_size=(181//2+1, 217//2+1, 181//2+1), embed_dim=256, num_classes=16, in_channels=1, drop_rate=0.5, depth=1, mlp_ratio=2., representation_size=None, uniform_drop=False, drop_path_rate=0.6, norm_layer=False, dropcls=0.25):
        super().__init__()
        ...
        self.embedding = nn.Linear(self.embed_dim, 64)
